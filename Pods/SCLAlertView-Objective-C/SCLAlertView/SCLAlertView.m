...
@property (nonatomic,copy)MyCustomBlockType completionBlockForWaiting;
...
- (void)viewWillLayoutSubviews
{
    ...
    NSLog(@"%@,%f",@"重新布局subview",kCircleIconHeight);
...
- (void)showWaiting:(UIViewController *)vc title:(NSString *)title subTitle:(NSString *)subTitle closeButtonTitle:(NSString *)closeButtonTitle duration:(NSTimeInterval)duration block:(MyCustomBlockType)completionBlock
{
    [self showTitle:vc image:nil color:nil title:title subTitle:subTitle duration:duration completeText:closeButtonTitle style:Waiting];
    self.completionBlockForWaiting=completionBlock;
}
-(void)executeCompletionBlockForWaiting{
    if (self.completionBlockForWaiting) {
        self.completionBlockForWaiting();
    }
    self.completionBlockForWaiting = ^(void) {
        ;
};
    [self fadeOut];
}
