...
- (void)viewDidLoad {
    ...
    _scene.backgroundColor=[UIColor colorWithRed:0xe0/255.f green:0xf8/255.f blue:0xd8/255.f alpha:1];
...
- (IBAction)tapAction:(UITapGestureRecognizer *)sender {
    ...
    [tappedNodes enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if ([tappedNode.name isEqualToString:@"Start"]||[tappedNode.name isEqualToString:@"FirstScreenGoShake"]) {
            ...
            if(!_imagesLoaded){
                [_alertView showWaiting:self title:[NSString stringWithFormat:@"%@...",NSLocalizedString(@"waiting", "等待语")] subTitle:[NSString stringWithFormat:@"%@...",NSLocalizedString(@"loading", "加载")] closeButtonTitle:nil duration:0.0 block:^{
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        bool shake=[tappedNode.name isEqualToString:@"FirstScreenGoShake"];
                        if (shake) {
                            [navigationController showMainControllerWithShake];
                        }else{
