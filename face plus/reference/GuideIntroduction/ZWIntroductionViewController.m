...
- (void)addBackgroundViews
{
    ...
    [[[[self backgroundImageNames] reverseObjectEnumerator] allObjects] enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if ([obj myContainsString:@"intro_1"]) {
            NSURL*url=[[NSBundle mainBundle] URLForResource:obj withExtension:@"gif"];
...
- (CGSize)contentSizeOfScrollView
{
   // UIView *view = [[self scrollViewPages] firstObject];
    return CGSizeMake([UIScreen mainScreen].bounds.size.width*4, [UIScreen mainScreen].bounds.size.height);
}
